...
function getValidNeighbors(blockPos: Vec3, distance: number): VienBlock[] {
	...
	result.push(new VienBlock(up, distance),
				new VienBlock(down, distance));
...
export default function get_default_world_spawn(world: World): Vec3 {
	...
	overworld.runCommand("say fixing a big bug... :D");
	...
	while (true) {
		if (overworld.getBlock(defaultWorldSpawn)?.isAir) {
			...
			if (!overworld.getBlock(defaultWorldSpawn)?.isAir) {
				...
				overworld.runCommand("say finneshed");
